...
class Cats:
    ...
    def Normal_Cat(cls):  # 第一種普通貓
        normal_cat = cls(NORMAL_CAT_IMAGE, 'normal',
                         health=50, damage=20, attack_range=100, stride=1.2, cost=10)
        return normal_cat

    @classmethod
    def Mask_Cat(cls):  # 第二種口罩貓
        mask_cat = cls(MASK_CAT_IMAGE, 'mask',
                       health=200, damage=25, attack_range=120, stride=1, cost=20)
        return mask_cat

    @classmethod
    def Sanitizer(cls):
        sanitizer = cls(SANI_CAT_IMAGE, 'sanitizer',
                        health=300, damage=50, attack_range=150, stride=1, cost=30)
        return sanitizer
